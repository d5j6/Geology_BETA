...
public class CharacterAnimControl : MonoBehaviour {
	...
	int animationIndex; //Переменная для фреймов
	int mNumAnimationFrames = 3645; //Количество кадров
	...
	void Update () {
		h = Input.GetAxis ("Horizontal"); //Отслеживание нажатий клавиш W/S (вперёд/надаз) (W: 0,1; S: -1,0)
		v = Input.GetAxis ("Vertical");  //Отслеживание нажатий клавш A/D (влево/вправо) (A: -1,0; D: 0,1);
		...
		animationIndex = ((int)(animator.GetCurrentAnimatorStateInfo(0).normalizedTime * (mNumAnimationFrames))) % mNumAnimationFrames; //подсчёт кадров
		print (animationIndex); //вывод текущего кадра
	...
	void FixedUpdate(){
		...
		if(Input.GetAxis ("Vertical")!=0){ //Проверка нажатия клавиш вперёд/назад
