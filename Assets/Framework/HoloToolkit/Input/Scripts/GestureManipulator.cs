...
{
    public enum WantedManipulation { Move, Rotate, Scale }
    ...
    public class GestureManipulator : MonoBehaviour
    {
        ...
        public WantedManipulation WantedManipulation = WantedManipulation.Move;

        float scaleAdding;
        float rotateAdding;
        public float RotationSensitivity = 60.0f;
        public float ScaleSensitivity = 3.0f;
        public float MaxScale = 2f;
        public float MinScale = 0.1f;

        public System.Action OnManipulationStarted;
        public System.Action OnManipulationEnded;
        ...
        void Update()
        {
            if (Manipulating)
            {
                switch (WantedManipulation)
                {
                    case WantedManipulation.Move:
                        ...
                        else
                        {
                            ...
                            //transform.position = Quaternion.Euler(new Vector3(0, Camera.main.transform.rotation.eulerAngles.y, 0)) * worldObjectPosition;
                        }
                        break;
                    case WantedManipulation.Rotate:
                        if (targetInterpolator != null)
                        {
                            //rotateAdding += -1 * Quaternion.Euler(new Vector3(GestureManager.Instance.ManipulationOffset.x, Camera.main.transform.rotation.eulerAngles.y, 0)).eulerAngles.x * RotationSensitivity * Time.deltaTime;
                            rotateAdding += -1 * GestureManager.Instance.ManipulationHandPosition.x * RotationSensitivity;
                            Vector3 eulRot = initialObjectRotation.eulerAngles;
                            eulRot.y += rotateAdding;
                            targetInterpolator.SetTargetRotation(Quaternion.Euler(eulRot));
                            //targetInterpolator.SetTargetRotation(Quaternion.Euler(eulRot));
                        }
                        else
                        {
                            //transform.Rotate(new Vector3(0, -1 * Quaternion.Euler(new Vector3(GestureManager.Instance.ManipulationOffset.x, Camera.main.transform.rotation.eulerAngles.y, 0)).eulerAngles.x * RotationSensitivity * Time.deltaTime, 0));
                            transform.Rotate(new Vector3(0, -1 * GestureManager.Instance.ManipulationOffset.x * RotationSensitivity * Time.deltaTime, 0));
                            //transform.Rotate(new Vector3(0, -1 * GestureManager.Instance.ManipulationHandPosition.x * RotationSensitivity, 0));
                        }
                        break;
                    case WantedManipulation.Scale:
                        //Debug.Log("GestureManager.Instance.ManipulationOffset.y = " + GestureManager.Instance.ManipulationOffset.y);
                        scaleAdding += GestureManager.Instance.ManipulationOffset.y * ScaleSensitivity * Time.deltaTime;
                        
                        //scaleAdding += GestureManager.Instance.ManipulationHandPosition.y * ScaleSensitivity * Time.deltaTime;
                        ...
                        {
                            transform.localScale = initialObjectScale * (1 + scaleAdding);
                        }
                        break;
                }
