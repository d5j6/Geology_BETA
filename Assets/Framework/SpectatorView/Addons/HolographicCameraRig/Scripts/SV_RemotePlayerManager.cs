...
namespace SpectatorView
{
    ...
    public class SV_RemotePlayerManager : SV_Singleton<SV_RemotePlayerManager>
    {
        ...
        private SV_Camera SV_Cam = null; // global link to SV_Camera
        ...
        void Update()
        {
            ...
            {
                // set callbacks
                SharingStage.Instance.SessionUsersTracker.UserJoined += SessionUsersTracker_UserJoined; // user_joined
                SharingStage.Instance.SessionUsersTracker.UserLeft += SessionUsersTracker_UserLeft; // user_left
            ...
            if (customMessages == null)
            {
                customMessages = SV_CustomMessages.Instance; // get link to custom_messages
        ...
        {
            // setup events
            customMessages.MessageHandlers[SV_CustomMessages.TestMessageID.HeadTransform] = UpdateHeadTransform;
            customMessages.MessageHandlers[SV_CustomMessages.TestMessageID.NetworkRoundTripTime] = OnNetworkRoundTripTime;

#if UNITY_EDITOR
            customMessages.MessageHandlers[SV_CustomMessages.TestMessageID.SpatialMapping] = OnSpatialMapping;
            customMessages.MessageHandlers[SV_CustomMessages.TestMessageID.UpdateSpectatorViewIP] = OnUpdateSpectatorViewIP;
#else
            customMessages.MessageHandlers[SV_CustomMessages.TestMessageID.SetNetworkTime] = OnNetworkTimeSet;
            customMessages.MessageHandlers[SV_CustomMessages.TestMessageID.SetColorDuration] = OnColorDuration;
            customMessages.MessageHandlers[SV_CustomMessages.TestMessageID.SpatialMappingRequest] = OnSpatialMappingRequest;
            customMessages.MessageHandlers[SV_CustomMessages.TestMessageID.TimeOffset] = OnTimeOffset;
            customMessages.MessageHandlers[SV_CustomMessages.TestMessageID.EditorUser] = OnEditorUser;
#endif
        }
        ...
        {
#if UNITY_EDITOR
            if (SV_CustomMessages.Instance != null
                && SV_CustomMessages.Instance.Initialized
                && SharingStage.Instance != null)
            {
                SV_CustomMessages.Instance.SendNetworkRoundTripTime(GetCurrentUnityTime());
            }
#endif
        }
        ...
        void OnNetworkRoundTripTime(HoloToolkit.Sharing.NetworkInMessage msg)
        {
...
#if !UNITY_EDITOR
            SV_CustomMessages.Instance.SendNetworkRoundTripTime(time);
#else
                ...
                && userID == HolographicCameraManager.Instance.tppcUser.GetID())
            {
                ...
                SV_CustomMessages.Instance.SendTPPCNetworkTime(NetworkTime);
            }
#endif
        }
        ...
        public RemoteHeadInfo GetRemoteHeadInfo(long userID)
        {
            ...
            if (headInfo.IP != null 
                ...
                && SharingStage.Instance.SessionUsersTracker != null)
            {
                // detect what SV user is connected
        ...
        void UpdateHeadTransform(NetworkInMessage msg)
        {
            ...
            if (headInfo.HeadObject != null)
            {
                    ...
                    && HolographicCameraManager.Instance.tppcUser.GetID() == userID)
                {
                    ...
                    if (headRot != Quaternion.identity) // prevent bag with slow wifi
                    {
        ...
        private void SendSpatialMappingData()
        {
            ...
            foreach (MeshFilter filter in meshFilters)
            {
                ...
                // broadcast spatial mapping data thru all devices
                SV_CustomMessages.Instance.SendSpatialMapping(vertices,
                                                              normals,
                                                              indices);
            }
        }
