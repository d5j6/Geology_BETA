...
{
	v.vertex = float4(cos(_Closed*(1 - (v.vertex.r + 0.5f))*6.28318530718f)*sin((v.vertex.b + 0.5f)*3.14159265f)*0.5*_Diameter, cos((v.vertex.b + 0.5f)*3.14159265f) *-0.5*_Diameter, sin(_Closed*(1 - (v.vertex.r + 0.5f))*6.28318530718f)*sin((v.vertex.b + 0.5f)*3.14159265f)*0.5*_Diameter, 1);
...
float4 frag(v2f i) : SV_Target
{
    float4 c;
		...
		half2 coods = half2(1.0h - ((1.0h - i.texcoord.r)*_Closed) - _RotationAngle / 360.0h, i.texcoord.g);
