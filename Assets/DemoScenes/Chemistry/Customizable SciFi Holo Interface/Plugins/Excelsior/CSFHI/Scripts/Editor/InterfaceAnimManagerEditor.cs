...
public class InterfaceAnimManagerEditor : Editor {
    ...
    InterfaceAnimManager _target;

    void OnEnable() {
        _target = (InterfaceAnimManager)target;

    }
    private void UpdateAnimClips() {
    ...
    public override void OnInspectorGUI() {
        ...
        */
        ...
        if (Application.isPlaying) {
            ...
            if (_target.currentState == CSFHIAnimableState.disappearing) {
                GUI.enabled = false;
            } else {
                GUI.enabled = true;
            }
        ...
        if (!_target.transform.parent) { //in case we select a prefab in the file structure
            return;
        }
        if (_target.transform.parent.GetComponent<InterfaceAnimManager>()) {
    ...
    private void displayElementList(bool isAppear) {
