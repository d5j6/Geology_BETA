...
public class HighlightOrbitElectrons : ActorActionClip
{
    ...
    protected override void OnEnter()
    {
        ...
        if(applyToChild)
        {
            _electrons = actor.transform.GetChild(childIndex).GetComponent<Atom>().GetOrbitElectrons(orbitIndex);
        }
        else
        {
            _electrons = actor.GetComponent<Atom>().GetOrbitElectrons(orbitIndex);
        }
